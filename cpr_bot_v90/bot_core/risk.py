...
class RiskManager:
        ...
        async with self.bot.lock:
            ...
            try:
                ...
                if (dist_l4 < 0.3 or dist_h4 < 0.3) and time.time() % 10 < 2:
                     logging.info(f"[{self.config.symbol}] 🔍 Cerca Nivel. Vol:{'✅' if vol_ok else '❌'}")
    ...
    async def check_position_state(self):
        async with self.bot.lock:
            try:
                ...
                # 1. ZOMBIE KILLER (Cierre Total)
                # Si la cantidad es 0 (o polvo), y el bot cree que está dentro -> LIMPIEZA TOTAL
                if qty < 0.0001:
                    if self.state.is_in_position:
                        logging.info(f"[{self.config.symbol}] Cierre detectado (Qty={qty}). Limpiando residuos...")
    ...
    async def _handle_full_close(self):
        ...
        # --- 1. LIMPIEZA DE EXCHANGE (CRÍTICO: Cancelar órdenes colgadas) ---
        try:
            logging.info(f"[{self.config.symbol}] Cancelando TODAS las órdenes abiertas...")
        ...
        # 2. Limpieza de Estado (State First)
    ...
    # ... (Resto de métodos igual: _check_trailing_stop, _handle_partial_tp) ...
