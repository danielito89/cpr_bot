...
class RiskManager:
    ...
    async def seek_new_trade(self, kline):
        ...
        if not can_open:
            # Loguear bloqueo de riesgo (Solo 1 vez por minuto)
        ...
        async with self.bot.lock:
            ...
            try:
                ...
                # Log si el precio está cerca de un nivel CLAVE (0.3%)
    ...
    async def check_position_state(self):
        async with self.bot.lock:
            try:
                ...
                # 1. Reconciliación (Si hay posición real pero no en memoria)
                ...
                # 2. CIERRE TOTAL / ZOMBIE KILLER (Prioridad Máxima)
                # Si la cantidad es casi cero, ejecutamos limpieza
                if qty < 0.0001:
    ...
    async def _handle_full_close(self):
        ...
        # 3. Reportar (Si falla, no bloquea el bot)
        pnl = 0.0
        roi = 0.0
        try:
