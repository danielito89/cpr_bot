#!/usr/bin/env python3
# backtester_v6.py
# Versión: v6.0 (Realismo Institucional)
# - Ejecución en NEXT OPEN (Elimina look-ahead bias).
# - Slippage Dinámico (Penaliza tamaño vs liquidez).
# - Costos de Fricción Completos.
...
# --- PARÁMETROS DE REALISMO (TUS MEJORAS) ---
# Slippage Base (Spread mínimo del exchange)
BASE_SLIPPAGE = 0.0005  # 0.05%
# Impacto de Mercado (Slippage dinámico por tamaño)
IMPACT_COEF = 0.001     # Coeficiente de impacto
MAX_IMPACT = 0.03       # Máximo castigo posible (3%)
# --------------------------------------------

# Estrategia a Probar
VOLUME_FACTOR = 1.1          
STRICT_VOLUME_FACTOR = 10.0   # Probemos si el "10" sobrevive a la realidad
...
class MockOrdersManager:
    def __init__(self, simulator):
        self.sim = simulator
    # En lugar de ejecutar, "Encolamos" la orden para la siguiente vela
...
class SimulatorState:
    def __init__(self):
        ...
        self.trades_history = []
        self.pending_order = None # Orden esperando apertura de vela
...
class BacktesterV6:
    ...
    # Paso 1: Recibir la intención del RiskManager (Fin de Vela N)
    ...
    # Paso 2: Ejecutar en la Apertura (Inicio de Vela N+1)
    def execute_pending_order(self, open_price, median_vol):
        order = self.state.pending_order
        self.state.pending_order = None # Limpiar
        
        # Calcular Impacto de Mercado (Slippage Dinámico)
        notional = order['qty'] * open_price
        vol_ratio = (notional / max(1.0, median_vol))
        
        # Fórmula de impacto: Coef * (Ratio ^ 0.8)
        impact = min(MAX_IMPACT, IMPACT_COEF * (vol_ratio**0.8))
        total_slippage = BASE_SLIPPAGE + impact
        
        # Precio Real de Entrada
        if order['side'] == SIDE_BUY:
            real_entry = open_price * (1 + total_slippage)
        else:
            real_entry = open_price * (1 - total_slippage)

        # Chequeo de Gap Mortal: Si el precio de apertura ya cruzó el SL, es pérdida instantánea
        ...
        self.state.current_position_info = {
            ...
            "slippage_impact": total_slippage # Guardar para análisis
        }
        ...
        if sl_hit_immediately:
            ...
            self.close_position("Gap/Slippage Kill")
    ...
    def close_position(self, reason):
        ...
        # Slippage Base en salida
        if info['side'] == SIDE_BUY: real_exit = exit_price_raw * (1 - BASE_SLIPPAGE)
        else: real_exit = exit_price_raw * (1 + BASE_SLIPPAGE)
        ...
        self.state.trades_history.append({
            ...
            'impact': info.get('slippage_impact', 0)
        })
    ...
    async def run(self):
        print(f"Iniciando Backtest V6.0 (NEXT OPEN + DINÁMICO) para {SYMBOL_TO_TEST}...")
        print(f"Vol: {VOLUME_FACTOR} | Estricto: {STRICT_VOLUME_FACTOR}")
        ...
        for row in df_merged.itertuples():
            ...
            self.state.current_price = row.Close # Precio de referencia para señales
            
            # --- PASO 1: EJECUTAR ÓRDENES PENDIENTES EN EL OPEN ---
            if self.state.pending_order:
                # Usamos el OPEN de la vela actual para ejecutar lo que decidió la vela anterior
            ...
            # Gestión de Posición (Exits usando High/Low de la vela actual)
            if self.state.is_in_position:
            ...
            # Generación de Señal (Al Cierre, para la siguiente vela)
    ...
    def print_results(self):
        ...
        avg_slippage = df['impact'].mean() * 100 if 'impact' in df.columns else 0

        print("\n" + "="*50)
        print(f" RESULTADOS V6.0 REALISTA: {SYMBOL_TO_TEST}")
        print(f" Slippage Dinámico Promedio: {avg_slippage:.4f}%")
        print("-" * 50)
        ...
        print("="*50)
        df.to_csv(os.path.join(DATA_DIR, f"backtest_v6_{SYMBOL_TO_TEST}.csv"))
