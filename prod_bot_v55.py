...
class AsyncTradingBotV55:
    def __init__(
        ...
        volume_factor=1.3,      # <-- CAMBIO: Ajustado a 1.3
        ...
        range_tp_mult=2.0,
        ema_period=20,          # <-- CAMBIO: Ajustado a 20 (EMA20)
        ema_timeframe="1h",
    ):
        self.symbol = symbol
    ...
    # --- Mensaje /pivots MEJORADO (CON H1-H6) ---
    ...
    # --- CÁLCULO DE PIVOTES MEJORADO (CON DEBUG Y FÓRMULA CLÁSICA) ---
    @tenacity_retry_decorator_async()
    ...
    async def seek_new_trade(self, kline):
        ...
        async with self.lock:
            try:
                ...
                # --- INICIO: LÓGICA DE TRADING MODIFICADA ---
                ...
                # --- FIN: LÓGICA DE TRADING MODIFICADA ---
        ...
        while self.running:
            try:
                ...
                if pos:
                    ...
                    # --- Bloque CIERRE DE POSICIÓN MEJORADO (CON ROI) ---
